...
The platform provides role-specific user interfaces designed for intuitive interaction across agricultural stakeholders, supporting both credential authentication and Web3 Sui wallet integration. This section presents the core operational interfaces that realize the system's security, blockchain anchoring, and custody transfer functions.

\figref{fig:farmer_dashboard} presents the Farmer Dashboard, which serves as the primary operational monitoring interface for agricultural producers. The interface displays real-time micro-climate telemetry metric cards (soil temperature, soil moisture, and soil pH) alongside active produce batches anchored to the Sui blockchain and dynamic yield forecast summaries.
...
\figref{fig:transfer_page} illustrates the Custody Transfer interface utilized by logistics handlers to record physical produce handoffs. The interface captures transit metadata (vehicle fleet registration, driver identity, and destination terminal) and executes \texttt{agri\_ledger::transfer\_custody} on-chain to reassign custody to the carrier's wallet address, ensuring transparent multi-hop traceability.
...
\figref{fig:audit_logs_page} depicts the Administrator Audit Log interface, providing a chronological, tamper-evident record of all system events, authentication attempts, and blockchain transactions. Each entry is cryptographically bound to an originating user UUID, IP address, timestamp, and SHA-256 digest for regulatory compliance and audit verification.
...
\figref{fig:prediction_analytics} illustrates the end-to-end architecture and decision workflow of the predictive analytics engine.
...
\figref{fig:yield_prediction_page} presents the operational yield prediction and decision support interface. The interface operationalizes the mathematical modeling formulation described in Equations~\ref{eq:mean_calc}--\ref{eq:conf_score}, displaying the estimated crop yield in metric tons, the computed confidence score, the micro-climate variance index, and multi-year projection curves with dynamic uncertainty bounds and actionable agronomic recommendations.
