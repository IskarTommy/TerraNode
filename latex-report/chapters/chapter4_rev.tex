...
The platform provides dedicated, role-specific user interfaces for administrators, farmers, and logistics handlers, as illustrated in Figures~\ref{fig:landing_page} through~\ref{fig:audit_logs_page}.
...
\hline
\textbf{Mean Temp ($\bar{T}$, $^\circ$C)} & \textbf{Mean Moisture ($\bar{M}$, \%)} & \textbf{Predicted Yield ($\hat{Y}$, MT)} & \textbf{Contextual Decision Recommendation} \\
\hline
